...
EDGE_VOICE_MAP = {
    ...
    "uk-UA": "uk-UA-PolinaNeural",
    "uk-UA": "uk-UA-PolinaNeural",
    "edge-de": "uk-UA-PolinaNeural",
    "edge-de-conrad": "uk-UA-OstapNeural",
    "edge-at": "uk-UA-PolinaNeural",
    "edge-ch": "uk-UA-PolinaNeural",
}
GOOGLE_VOICE_MAP = {
    "uk-UA": {
        "languageCode": "uk-UA",
...
def translate_text_to_chinese(text):
    return translate_text(text, "uk|zh-CN")


def translate_text_to_english(text):
    return translate_text(text, "uk|en")
...
def lookup_dictionary_entry(word):
    if DEFAULT_VOICE == "uk-UA":
        raise ValueError("没有查到这个乌克兰语词的在线词典释义")
    ...
    try:
        ...
        if exc.code == HTTPStatus.NOT_FOUND:
            raise ValueError("没有查到这个乌克兰语词的在线词典释义") from exc
        raise
...
def lookup_translation_fallback(word):
    ...
    fallback_definition = normalize_space(
        f"bedeutet ungefähr {translated_en}" if translated_en else f"ukrainisches Wort oder Ausdruck: {normalize_space(word)}"
    )
    note_parts = ["机器翻译兜底"]
